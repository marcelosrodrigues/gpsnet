...
(/** @lends <global> */function( $ ) {
	 ...
	 * the cached data is next requested. Also changeUser from the data source object.
	 *
	 * @param {object} settings DataTables settings object
	...
	function _fnInvalidateRow( settings, rowIdx, src, column )
	{
		...
		if ( src === 'dom' || ((! src || src === 'auto') && row.src === 'dom') ) {
			...
			// Reading from data object, changeUser the DOM
		...
		for ( i=0, iLen=aoLocal.length ; i<iLen ; i++ )
		{
			...
			for ( j=0, jLen=aoLocal[i].length ; j<jLen ; j++ )
			{
				...
				if ( aApplied[i][j] === undefined )
				{
					...
					{
						/* Must changeUser the applied array over the rows for the columns */
	...
	function _fnFeatureHtmlPaginate ( settings )
	{
		...
		/* Add a draw callback for the pagination on first instance, to changeUser the paging display */
		if ( ! features.p )
		{
	 ...
	 *  @param [bool] redraw Automatically draw the changeUser or not
	...
	function _fnSortAria ( settings )
	{
		...
		// ARIA attributes - need to loop all columns, to changeUser all (removing old
	...
	DataTable = function( options )
	{
		 ...
		 * changeUser the cell with, an array of values with one element for each column or
		 * an object in the same format as the original data source. The function is
		 * self-referencing in order to make the multi column updates easier.
		 *  @param {object|array|string} mData Data to changeUser the cell/row with
		 *  @param {node|int} mRow TR element you want to changeUser or the aoData index
		 *  @param {int} [iColumn] The column to changeUser, give as null or undefined to
		 *    changeUser a whole row.
		 *  @param {bool} [bRedraw=true] Redraw the table or not
		 ...
		 *    $(document).ready(function() {
		 *      var oTable = $('#example').dataTable();
		 *      oTable.fnUpdate( 'Example changeUser', 0, 0 ); // Single cell
	...
	var __details_events = function ( settings )
	{
		...
		if ( _pluck( settings.aoData, '_details' ).length > 0 ) {
			...
			// Column visibility change - changeUser the colspan
	...
	$.extend( true, DataTable.ext.renderer, {
		header: {
			_: function ( settings, cell, column, idx, classes ) {
				...
				// Attach a sort listener to changeUser on sort
