...
<div class="col-lg-12">
    ...
    <form action="<c:url value='/empresa/pesquisar.do'/>" role="form">
        <div class="row">
            <div class="col-sm-4">
                <div class="form-group">
                    <input type="text" name="object.nome" id="object.nome" placeholder="<fmt:message key='empresa.nome' bundle='${labels}'  />" class="form-control" />
                </div>
            </div>
            <div class="col-sm-3">
                <div class="form-group">
                    <select name="object.endereco.estado" id="object.endereco.estado" class="form-control">
                        ...
                        <c:forEach items="${estados}" var="estado">
                            ...
                            <c:if test="${estado.uf.equals(param['object.endereco.estado'])}">selected="selected"</c:if>>${estado.nome}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
    ...
    <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th><fmt:message key="empresa.nome" bundle="${labels}"/></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${resultlist.list}" var="object">
                    <tr>
                        <td><a href='<c:url value="/empresa/${object.id}/abrir.do" />'>${object.id}</a></td>
                        <td>${object.nome}</td>
                    </tr>
                </c:forEach>
            </tbody>
                        ...
                        <ul class="pagination">
                            <li class="paginate_button previous <c:if test='${not resultlist.previous}'>disabled</c:if>" aria-controls="dataTables-example" tabindex="0"
                                ...
                                <c:if test='${resultlist.previous}'>
                                    <a href="<c:url value='/empresa/pesquisar.do'/>?object.nome=${param['object.nome']}&page=${resultlist.page - 1}">&lt;&lt;</a>
                                </c:if>
                            </li>

                                <c:forEach begin="1" end="${resultlist.pageCount}" var="index">
                                    <li class="paginate_button" tabindex="${index - 1}">
                                        <a href="<c:url value='/empresa/pesquisar.do'/>?object.nome=${param['object.nome']}&page=${index -1}">${index}</a>
                                    </li>
                                </c:forEach>
        ...
        <div class="row" style="margin-top: 10px;">
            <div class="col-sm-6">
                <a href="<c:url value='/empresa/novo.do' />" class="btn btn-info">Adicionar</a>
            </div>
        </div>
